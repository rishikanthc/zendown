	...
	<main class="flex-1 flex flex-col min-h-screen">
			...
			<div class="flex items-center justify-between">
				<div class="flex items-center gap-2 sm:gap-4">
					...
					<div class="flex items-center gap-2 sm:gap-3">
						<div>
							<h2 class="text-base sm:text-lg font-semibold text-foreground">Images</h2>
		...
		<div class="flex-1 p-3 sm:p-6">
			...
			{:else if attachments.length === 0}
				<div class="flex flex-col items-center justify-center h-48 sm:h-64 text-center">
					<svg class="w-12 h-12 sm:w-16 sm:h-16 text-muted-foreground/50 mb-3 sm:mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
						<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"></path>
					</svg>
					<h3 class="text-base sm:text-lg font-semibold text-foreground mb-1 sm:mb-2">No images yet</h3>
					<p class="text-sm text-muted-foreground mb-3 sm:mb-4 px-4">Upload images in your notes to see them here</p>
					<a href="/">
					...
					{#each attachments as attachment}
						<div class="group cursor-pointer" onclick={() => openImage(attachment)}>
							<div class="relative aspect-square overflow-hidden rounded-lg bg-muted">
								...
								<div class="absolute top-1 sm:top-2 right-1 sm:right-2 flex gap-0.5 sm:gap-1">
									<!-- Copy button -->
									<button
										onclick={(e) => copyMarkdownSyntax(attachment, e)}
										class="w-6 h-6 sm:w-8 sm:h-8 bg-blue-500 hover:bg-blue-600 text-white rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-200 shadow-lg hover:shadow-xl"
									...
									<button
										...
										class="w-6 h-6 sm:w-8 sm:h-8 bg-red-500 hover:bg-red-600 text-white rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-200 shadow-lg hover:shadow-xl disabled:opacity-50 disabled:cursor-not-allowed"
										title="Delete image"
									>
							...
							<div class="mt-1 sm:mt-2 px-0.5 sm:px-1">
								<h3 class="font-medium text-xs text-foreground truncate" title={attachment.original_name}>
									{attachment.original_name}
								</h3>
								<div class="flex items-center justify-between text-xs text-muted-foreground mt-0.5 sm:mt-1">
									<span>{formatFileSize(attachment.size)}</span>
									<span class="hidden sm:inline">{formatDate(attachment.created_at)}</span>
									<span class="sm:hidden">{new Date(attachment.created_at).toLocaleDateString()}</span>
								</div>
							</div>
						</div>
					{/each}
...
{#if isImageModalOpen && selectedImage}
	<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4" onclick={closeImageModal}>
		<div class="relative w-full max-w-4xl max-h-[90vh]" onclick={(e) => e.stopPropagation()}>
			<!-- Close button -->
			<button 
			...
			<div class="bg-white rounded-lg shadow-2xl overflow-hidden">
				...
				<div class="p-3 sm:p-4 bg-white">
					<h3 class="font-semibold text-base sm:text-lg text-foreground mb-2 truncate">{selectedImage.original_name}</h3>
					<div class="grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-4 text-xs sm:text-sm text-muted-foreground">
						<div>
						...
						<div>
							...
							<br>
							<span class="break-all">{selectedImage.mime_type}</span>
						</div>
						<div>

...
export const POST: RequestHandler = async ({ request, locals }) => {
	...
	if (!locals.user) {
		return json(
			{ message: 'Unauthorized. You must be logged in to create a note.' },
			{ status: 401 }
		);
	}
	...
	try {
		...
		if (!insertedResult) {
			...
			return json(
				{
					message: 'Failed to create new note due to an unexpected server error.'
				},
				{ status: 500 }
			);
		}
		...
		if (insertedResult && insertedResult.id && typeof insertedResult.content === 'string') {
			try {
				...
				if (!aiResponse.ok) {
					...
					console.log(`Successfully upserted note ${insertedResult.id} to AI server.`);
				}
			} catch (aiError) {
				// Log network or other errors calling AI server but don't fail the main operation
				console.error(`Failed to call AI server for upserting note ${insertedResult.id}:`, aiError);
			}
		} else {
			console.warn(
		...
		return json(
			{
				message: 'An unexpected error occurred on the server while creating the note.'
			},
			{ status: 500 }
		);
	}
